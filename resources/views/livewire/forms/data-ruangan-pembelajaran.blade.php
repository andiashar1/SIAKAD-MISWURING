<div class="p-6">
    ...
    <x-header title="Jadwal Pelajaran" subtitle="Dashoboard / Jadwal Pelajaran">
    ...
    <div class="bg-white rounded-md border border-gray-100 p-6 shadow-md shadow-black/5 ">
        <x-table :headers="$headers" :rows="$tampil" with-pagination>
            ...
            {{ $tampil->jumlah_siswa}} Mapel
            @endscope
            @scope('actions', $tampil)
            <div class="flex justify-start" >
                <x-button icon="s-table-cells" link="/jadwal_pelajaran/{{$tampil->id_wl}}/tambah" spinner class="btn-xs text-green-400 btn-ghost" no-wire-navigate />
            </div>
            @endscope
        </x-table>
