<div class="p-6">
    <x-toastr/>
    <x-confirm-deleted/>
    <x-header title="Guru" subtitle="Check this on mobile">
            ...
            <x-slot:actions>
                ...
                <x-button icon="o-plus" class="btn-primary btn-sm" link="{{ route('guru.tambah')}}" no-wire-navigate />
            </x-slot:actions>
        </x-header>
    <div class="bg-white rounded-md border border-gray-100 p-6 shadow-md shadow-black/5 ">
        <x-table :headers="$headers" :rows="$tampil" with-pagination>
            ...
            @endscope
            @scope('header_nip', $header)
                <h3 class="text-sm font-bold text-transparent bg-gradient-to-r from-sky-600 to-cyan-400 bg-clip-text">
                    {{ $header['label'] }}
                </h3> 
            @endscope
            @scope('header_nuptk', $header)
                <h3 class="text-sm font-bold text-transparent bg-gradient-to-r from-sky-600 to-cyan-400 bg-clip-text">
                    {{ $header['label'] }}
                </h3> 
            @endscope
            @scope('cell_id_guru', $tampil)
            <div class="w-30 avatar">
                <div class="w-10 rounded-full border-2 border-blue-500">
                    <img src="{{asset('storage/'.$tampil->foto)}}"/> 
                </div>
            <span class=" ml-2 self-center"> {{$tampil->id_guru}}</span>
            </div>    
            @endscope
            @scope('actions', $user)
            <div class="flex justify-start" >
                <x-button icon="o-eye" wire:click="delete({{ $user->id }})" spinner class="btn-xs text-blue-700 btn-ghost" />
                <x-button icon="o-pencil-square" link="/guru/{{$user->id}}/edit" spinner class="btn-xs text-amber-400 btn-ghost" no-wire-navigate/>    
                <x-button icon="o-trash" wire:click="$dispatch('confirm-delete', { get_id: '{{ $user->id}}' })" spinner class="btn-xs text-red-500 btn-ghost" />
            </div>
            @endscope
        </x-table>
        <!-- filter -->
        <x-drawer wire:model="showFilters" title="Filters" subtitle="Data Guru" separator with-close-button class="w-11/12 lg:w-1/3" right>
    <div>
